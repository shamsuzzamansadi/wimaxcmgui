...
MainWindow::MainWindow(QWidget *parent) : QMainWindow(parent), ui(new Ui::MainWindow)
{
    ...
    ui->linkToProjectHome->setOpenExternalLinks(1);
    ui->linkToEmailUs->setOpenExternalLinks(1);
...
bool MainWindow::VerifyCommandPath(const char *cmd)
{
    QProcess *whereiscommand = new QProcess();
    whereiscommand->start(QString("which %1").arg(cmd));
    whereiscommand->waitForFinished(-1);
    QString commandbinpath = QString(whereiscommand->readAll()).trimmed();
    delete whereiscommand;
...
void MainWindow::CheckGraphicalSu()
{
    if(VerifyCommandPath("gksudo")) // Duh! :@
        GUIRoot = "gksudo";
    else if(VerifyCommandPath("kdesu"))
        GUIRoot = "kdesu";
    else if(VerifyCommandPath("gksu"))
        GUIRoot = "gksu";
    else if(VerifyCommandPath("kdesudo"))
        GUIRoot = "kdesudo";
    else if(VerifyCommandPath("gnomesu"))
        GUIRoot = "gnomesu";
}
...
{
    FILE *userdatafile = fopen(QString(QDir::homePath() + "/.bluserinfo").toAscii(),"r");
...
void MainWindow::CheckEthAlias()
{
    ...
    if(VerifyCommandPath("ifconfig"))
        ifconfig = popen(QString("ifconfig -a | grep -i %1 | cut -d' ' -f1").arg(HWAddress).toAscii(), "r");
    else if(VerifyCommandPath("/sbin/ifconfig"))
...
void MainWindow::SetStatusNoDevice()
{
    ...
    ui->NetworkQualityMeter->setPixmap(QPixmap(":/images/0"));
    ui->statusBar->showMessage("Device is unplugged", 3000);
...
void MainWindow::SetIP()
{
    ...
    FILE *inetaddr;
    if(VerifyCommandPath("/sbin/ifconfig"))
        inetaddr = popen(QString("/sbin/ifconfig %1 | grep 'inet addr:' | cut -d: -f2 | awk '{print $1}'").arg(Ethernet).toAscii(), "r");
    else
        inetaddr = popen(QString("ifconfig %1 | grep 'inet addr:' | cut -d: -f2 | awk '{print $1}'").arg(Ethernet).toAscii(), "r");
    ...
    if(isConnected)
        {
            ...
            pclose(wimaxcfreq);
            ui->statusBar->showMessage(QString("Connected To %1MHz").arg(freq/1000), 3000);
        }
    else
        ui->statusBar->showMessage("There Is No Active Connection", 3000);
}
...
void MainWindow::AccountTabJob()
{
    ...
    if(strcmp(ActiveUserID,"''") == 0)
        ui->statusBar->showMessage("No User ID Is Activated",3000);
    else
        ui->statusBar->showMessage("Current User ID: "+QString(ActiveUserID), 3000);
    free(ActiveUserID);
}
...
void MainWindow::on_buttonCD_clicked()
{
    if(isConnected)
        ...
        {
            int toConnectFreq = 0, maxCINR = -100, freq, cinr;
            char *buff = (char*)malloc(200);
            FILE *wimaxcsearch = popen("wimaxc search | grep 0x | cut -d'x' -f2-6","r");
            while(fgets(buff, 100, wimaxcsearch))
                {
                    sscanf(buff, "%*s %d.000 10.000 %*d %d", &freq, &cinr);
                    if( cinr > maxCINR )
                        {
                            maxCINR = cinr;
                            toConnectFreq = freq;
            ...
            if(toConnectFreq >= 2300 && toConnectFreq <= 3500)
                {
                    system(QString("wimaxc connect %1 10").arg(toConnectFreq).toAscii());
                    ui->statusBar->showMessage(QString("Signal Sent For %1MHz").arg(toConnectFreq), 3000);
                }
            else
                ui->statusBar->showMessage(QString("No BSID Found!"), 3000);
        }
}
    ...
    if ( ui->lineEditUID->text() != QString("") && ui->lineEditPasswd->text() != QString("") && ui->lineEditMAC->text() != QString(""))
        {
            if(ui->lineEditMAC->text().contains(" ") || ui->lineEditPasswd->text().contains(" ") || ui->lineEditUID->text().contains(" "))
                ...
                {
                    QString wimaxupdateconfig = QString("wimaxc set UserIdentity %1 && wimaxc set UserPassword %2 && wimaxc set TTLSAnonymousIdentity %3@banglalion.com.bd && wimaxc options save").arg(ui->lineEditUID->text()).arg(ui->lineEditPasswd->text()).arg(ui->lineEditMAC->text());
                    system(wimaxupdateconfig.toAscii());
    ...
    if ( ui->lineEditUID->text() != QString("") && ui->lineEditPasswd->text() != QString("") && ui->lineEditMAC->text() != QString(""))
        {
            if(ui->lineEditMAC->text().contains(" ") || ui->lineEditPasswd->text().contains(" ") || ui->lineEditUID->text().contains(" "))
            ...
            else
                {
                    ...
                    FILE *saveuserdata = fopen(QString(QDir::homePath() + "/.bluserinfo").toAscii(),"w");
                    for(int i = 0; i < UserIDList.size(); i+=3)
    ...
    if(!UserIDList.empty())
        {
            ...
            FILE *userdatafile = fopen(QString(QDir::homePath() + "/.bluserinfo").toAscii(),"w");
            for(int i = 0; i < UserIDList.size(); i+=3)
                fputs(UserIDList.at(i).toAscii()+ " " + UserIDList.at(i+1).toAscii() + " " + UserIDList.at(i+2).toAscii() + "\n", userdatafile);
            fclose(userdatafile);
        }
...
void MainWindow::on_buttonManualConnect_clicked()
{
    ...
    //QProcess::startDetached("konsole -e wimaxc -i search");
}
...
//////////////////////////////////// Auto Load Info When Selecting Profile ////////////////////////////////////
